...
namespace crackitos_physics::math
{
    ...
    struct FourVec2
    {
        ...
        FourVec2<T> operator*(const crackitos_physics::commons::fp scalar) const; //Multiply by scalar
        FourVec2<T> operator*(const std::array<T, 4> scalars) const; //Multiply by 4 scalars
        FourVec2<T> operator/(const crackitos_physics::commons::fp scalar) const; //Divide by scalar
        std::array<crackitos_physics::commons::fp, 4> Dot(const FourVec2<T>& other) const; //Dot
    ...
    using FourVec2f = FourVec2<crackitos_physics::commons::fp>;

    //TODO + LTO, use nodiscard, constexpr where needed

    //Specialization for physics923::commons::fp
    template <>
    FourVec2f FourVec2<crackitos_physics::commons::fp>::operator+(const FourVec2f& other) const;
    template <>
    FourVec2f FourVec2<crackitos_physics::commons::fp>::operator-(const FourVec2f& other) const;
    template <>

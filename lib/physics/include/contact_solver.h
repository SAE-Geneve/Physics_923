...
namespace physics923::physics
{
    ...
    private:
        ...
        void ResolveVelocities() const
        {
            ...
            float restitution = (collider_a.bounciness() * body_a.mass() + collider_b.bounciness() * body_b.mass())
                                / (body_a.mass() + body_b.mass());

            // If velocity is very small, set restitution to zero
            constexpr float restitution_threshold = 2.0f;
            ...
            float sleep_velocity_threshold = 10.0f;
